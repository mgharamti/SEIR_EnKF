...
model.Npop = 794045;      % population (GY: )
...
model.kappa   = 0.000308;   % Mortality rate   [kappa] e.g., 307.58 per 100,000 pop (source: John Hopkins)
model.tdea    = 7;          % Time until death  [1/rho]

model.mu      = 2.4776e-5;  % 7.7/1000;  % natural death rate [US: 8,295/Npop] (source: https://worldpopulationreview.com/countries/deaths-per-day)
...
model.beta1   = 0.25/model.Npop;   % .45transmission rate before intervention [lead to R > 1; unstable i.e., exponential growth]
...
model.alpha2  = 7e-3;    
model.sigma   = 0.785;       % .175vacc efficiency (pfizer: 95%, JJ: 70%) take average?!

% Periods for: Vaccination rate 
a1 = 0.0 * ones(1, model.Nt2);
a2 = 0.2 * ones(1, 100);
a3 = 0.5 * ones(1, 200);
a4 = 1.0 * ones(1, model.Nt - (300+model.Nt2));

model.A = [a1, a2, a3, a4] / 100;

% Periods for: Transmission rate (should related to lockdown/intervention)
b1 = 0.40 * ones(1, 50);        % ~first 50 days
b2 = 0.36 * ones(1, 50);        % approaching first lockdown
b3 = 0.30 * ones(1, 50);        % during lockdown (people start behaving themselves)
b4 = 0.35 * ones(1, 50);        % summer; things are opening up
b5 = 0.35 * ones(1, 50);        % mid summer 2020
b6 = 0.24 * ones(1, 250);       % fall, people getting sick again
b7 = 0.55 * ones(1, 200);       % delta wave
b8 = 0.50 * ones(1, 876-700);   % omicron wave
