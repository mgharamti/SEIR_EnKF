function [model, da, obs, diags, state] = DA_exps_GY(my_config)
...
% Initialize Model:
[model, x0] = initialize_seir_exps_GY(ti, tl, tv, tf, my_config);


% Data:
[~, obs.Active, obs.Recovered, obs.Deaths, obs.Vaccinated] = read_data_GY;


% DA Configuration:
[Ol, Y, R, Xa, da] = configure_DA_exps_GY(model, x0, obs.Active, obs.Recovered, obs.Deaths, obs.Vaccinated, my_config);
...
if my_config.results
    ...
    for o = 1:da.Ny
        figure
        plot(model.time, diags.RMSE(o, :), '*','Color', 'k', 'LineWidth', 1); hold on 
        plot(model.time, diags.RMSEf(o, :), '*','Color', bL, 'LineWidth', 1); 
        plot(model.time, diags.RMSEa(o, :), '*','Color', rD, 'LineWidth', 1);
        set(gca, 'FontSize', 14, 'YGrid', 'on')
